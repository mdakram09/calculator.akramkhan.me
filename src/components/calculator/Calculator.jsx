...
const Calculator = () => {
    ...
    const btnClick = (item) => {
        ...
        if (item.action === BTN_ACTIONS.CALC) {
            ...
            let facto = expression.charAt(expression.length-1)
            console.log(facto)
            let exp = expression.slice(0,3)

            if (facto === '!'){
                let que = Number(expression.slice(0,expression.length-1))
                console.log(que)
                let res=1
                for(let i=1;i<=que;i++){
                    res=res*i
            ...
            if (exp === 'sin'){
                let que = Number(expression.slice(3))
                let res = Math.sin(que);
                setExpression(res.toString());
            }
            if (exp === 'cos'){
                let que = Number(expression.slice(3))
                let res = Math.cos(que);
                setExpression(res.toString());
            }
            if (exp === 'tan'){
                let que = Number(expression.slice(3))
                let res = Math.tan(que);
                setExpression(res.toString());
            }
            try {
